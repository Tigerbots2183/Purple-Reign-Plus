...
import frc.robot.commands.MoveOffLine;
...
public class RobotContainer {
    ...
    private final POVButton blue = new POVButton(driver, 0);
    private final POVButton pink = new POVButton(driver, 180);
    private final POVButton green = new POVButton(driver, 90);
    private final POVButton orange = new POVButton(driver, 270);
    ...
    private final OneShotButton PLMbtn = new OneShotButton("LMbtn",
            () -> new AlignToPose(StationPOSES.Left_mid_station, drivetrain));
    ...
    private final OneShotButton PRMbtn = new OneShotButton("RMbtn",
            () -> new AlignToPose(StationPOSES.Right_mid_station, drivetrain));
    ...
    private void configureButtonBindings() {
        ...
        intakeButton.whileTrue(new Intake(-.07, s_CoralCom).withTimeout(Seconds.of(0.3)));
        ...
        // shootButton.whileTrue(new Intake(-0.07, s_CoralCom));
        ...
        blue.whileTrue(s_ledCom.runOnce(()->{
                s_ledCom.led(54);
        }));

        pink.whileTrue(s_ledCom.runOnce(()->{
                s_ledCom.led(52);
        }));

        orange.whileTrue(s_ledCom.runOnce(()->{
                s_ledCom.led(58);
        }));

        green.whileTrue(s_ledCom.runOnce(()->{
                s_ledCom.led(56);
        }));
    ...
    public Command getAutonomousCommand() {
        ...
        return new MoveOffLine(drivetrain);
    }
}
