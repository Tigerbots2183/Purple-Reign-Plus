...
public class Alignmentright extends Command {
  ...
  pidrotation = new PIDController(0.1, 00, 0.001);
  ...
  public void execute() {
    ...
    double[] positionr = LimelightHelpers.getTargetPose_CameraSpace("");
    
    double xSpeed = -pidx.calculate(position[0]);
    double ySpeed = -pidy.calculate(position[2]);
    //double rotValue = -pidrotation.calculate(positionr[4]);
    

    boolean atXSetpoint = Math.abs(position[0] - .12) <= .01;
    boolean atYSetpoint = Math.abs(-position[2] -.45) <= .015;
    //boolean atRSetpoint = Math.abs(-positionr[4] -0.5 ) <= .3;
      ...
      //   s_Swerve.drive(new Translation2d(0,ySpeed),  false,false);
      }else if(!atXSetpoint){
        s_Swerve.drive(new Translation2d(xSpeed,0), 0, false,false);
      }else if(!atYSetpoint){
